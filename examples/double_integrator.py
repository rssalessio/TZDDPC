...
e_simplified = [np.zeros_like(x_simplified[-1])]
Ze_simplified = [Zonotope(np.zeros(dim_x), np.zeros((dim_x,1))) + x_simplified[-1]]

for t in range(total_steps):
    result, v, xbark, Zek = szddpc.solve_simplified(
        xbar_simplified[-1],
        e_simplified[-1],
        horizon,
        Zsigma,
    ...
    print(f'[{t}] x: {x_simplified[-1]} - xbar: {xbar_simplified[-1]} - v: {v[0]}')

    xbar_simplified.append(xbark[1])
    u = szddpc.theta.K @ x_simplified[-1] + v[0]
    x_next = sys.A @ x_simplified[-1] +  np.squeeze(sys.B @ u) + W.sample()
    x_simplified.append(x_next.flatten())
    e_simplified.append(x_simplified[-1] - xbar_simplified[-1])
    
    Zek = Zek.Z.value
    Ze_simplified.append(Zonotope(Zek[:, 0], Zek[:, 1:]) + xbar_simplified[-1])

x_simplified = np.array(x_simplified)
xbar_simplified = np.array(xbar_simplified)
e_simplified = np.array(e_simplified)



fig, ax = plt.subplots()
colors = cm.gray(np.linspace(0, 1, len(Ze_simplified)))

centers = []
for idx, Z in enumerate(Ze_simplified):
    centers.append(Z.center)
    Z = Z.reduce(1)
    collection = PatchCollection([Z.polygon],  facecolor='lightgray', edgecolor='black', lw=0.5)
    ax.add_collection(collection)
...
plt.annotate('$x_0$',xy=(x_simplified[0,0]+0.05,x_simplified[0,1]+0.05),xytext=(x_simplified[0,:] + 0.5),
                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.),
                bbox=dict(pad=0, facecolor="none", edgecolor="none"))


plt.annotate(f'$x_{t+1}$',xy=(x_simplified[-1,0]-0.05,x_simplified[-1,1]-0.05),xytext=(x_simplified[-1,:] - 0.5),
                arrowprops=dict(arrowstyle='-|>', fc="k", ec="k", lw=1.),
                bbox=dict(pad=0, facecolor="none", edgecolor="none"))
plt.plot(x_simplified[:,0], x_simplified[:,1], linestyle='dashdot', marker='x', color='black', linewidth=0.7)
plt.plot(centers[:,0], centers[:,1], linestyle='solid', color='black', linewidth=0.15)
